import SDK, { type Note, type Collection } from "./api";
import axios from "axios";
import { programmingNotes, financeNotes } from "../mock-data/mockData";

const idOf = (x: Note | Collection): number =>
  (x.id ?? (x as Note).note_id ?? (x as Collection).collection_id) as number;

async function wipe() {
  const { data: notes } = await SDK.getNotes();
  ...
  console.log(
    `🧹 wipe: ${notes.length} note, ${cols.length} collection eliminate`,
  );
}
...
async function seed() {
  try {
    ...
    const collection1: Collection = await SDK.createCollection({
      name: "Programming notes",
    });
    const collection2: Collection = await SDK.createCollection({
      name: "Finance notes",
    });
    ...
    const insertNotesInCollection = async (
      notes: Note[],
      collectionId: number,
    ) => {
